...
from utils import (load_tess_dataset, get_splits, load_audio,
                   extract_mfcc, save_results, set_seed, get_device, NUM_CLASSES)
    ...
    def __getitem__(self, idx):
        ...
        if T < self.max_len:
            features = np.vstack([features,
                np.zeros((self.max_len - T, features.shape[1]), dtype=np.float32)])
        else:
...
def train(data_root, epochs=30, batch_size=32, lr=1e-3, results_dir='Results'):
    ...
    trl = DataLoader(SpeechDataset(tr), batch_size, shuffle=True,
    ...
    best  = 0.0
    spath = os.path.join(results_dir, 'speech_best_model.pt')
...
if __name__ == '__main__':
    ...
    args = p.parse_args()
    train(args.data_root, args.epochs, args.batch_size, args.lr, args.results_dir)